...
int rtspcl_add_exthds(rtspcl_t *p, char *key, char *data)
{
	...
	rtspcld=(rtspcl_data_t *)p;
	if(!rtspcld->exthds){
		if(realloc_memory((void*)&rtspcld->exthds, 17*sizeof(key_data_t)*2,__func__)) return -1;
	}else{
		i=0;
...
int rtspcl_setup(rtspcl_t *p, key_data_t **kd)
{
	...
	if (rtspcld->udp){
		...
		//hds[0].data=(__u8*)"RTP/AVP/TCP;unicast;interleaved=0-1;mode=record;control_port=6002;timing_port=6003";
	}
	else {
...
int rtspcl_teardown(rtspcl_t *p)
{
	...
	if(!p) return -1;
	rtspcld=(rtspcl_data_t *)p;
	return exec_request(rtspcld, "TEARDOWN", NULL, NULL, 0, 0, NULL, &rtspcld->kd, NULL);
}
		...
		char *content, int length, int get_response, key_data_t *hds, key_data_t **kd, char* url)
{
	...
	if( rtspcld->udp && rtspcld->session != NULL )
	{
		sprintf(reql,"Session: %s\r\n",rtspcld->session);
		strncat(req,reql,sizeof(req));
	}
